...
function LoginContent() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    const checkRes = await fetch("/api/auth/check-banned", {
      method: "POST",
    ...
    if (checkData.banned) {
      setLocalError("Your account has been banned. Contact support for assistance.");
      setLoading(false);
      return;
    }
    ...
    <div className="flex min-h-screen items-center justify-center bg-[#0a2540] px-6">
      <div className="w-full max-w-sm text-center">
        ...
        <button
          ...
          className="mb-6 flex w-full items-center justify-center gap-3 rounded-lg bg-white px-4 py-3 text-[14px] font-semibold text-[#0a2540] transition-all hover:bg-white/90"
        >
        ...
        <div className="mb-6 flex items-center gap-4">
          ...
          <span className="text-[12px] text-[#8898aa]">or sign in with email</span>
          <div className="h-px flex-1 bg-white/10" />
        </div>
